...
LLaDA は順方向と逆方向で対称な性能を示し、reversal curse (Berglund et al. 2023 [1]) を持たない。AR である Pythia-160M は典型的な逆向き劣化を示す。LLaDA と Pythia-160M の gap 差は +33.5 pp であり、本研究の unlearning が削減を試みる対象である。
...
上記の損失関数を αL_rs + retain·CE の合算として AdamW8bit で 1000 step 最小化する。retain 損失は WikiText-103 上の通常 MDM CE であり、一般言語能力の保持を担う。
...
Transformer 系列の階層別機能分業に関する先行研究 (Tenney et al. 2019 [5], Clark et al. 2019 [2]) は、表層的な構文処理が浅層に、意味的統合が深層に局在することを示している。本研究の予備観察でも、LLaDA の双方向情報統合は層 20 から層 32 までの深層に集中する傾向が認められた。Phase 3 v2 は 33 層を一様平均しており、深層への勾配が約三分の一に希釈されていた可能性がある。Phase 3 v3 では二点を改良した。
...
1. 完全な AR 復元には至らない: Phase 3 v3 の +14 pp は AR 参照点 +33.5 pp の四割程度であり、forward−reverse 非対称性は部分的にしか再現されない。
