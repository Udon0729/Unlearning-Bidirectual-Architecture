...
本リポジトリは、アーキテクチャを変更せず、unlearning のみにより拡散言語モデルを自己回帰モデルの挙動に戻せるかという問いを実証的に検討した研究プロジェクトの実装と結果を収録する。

## 研究の問い

既存研究 (Gong et al. 2024 [3]) は AR モデルから DLM への変換を追加学習またはアーキテクチャ拡張により実現してきた。本研究は逆向きの問いを扱う。すでに DLM 化されたモデルから、勾配計算と既存パラメータの選択的更新のみで AR の挙動を復元できるか。

上記の問いの解明は次の二点で意義がある。

1. DLM が獲得した双方向参照能力がどのような内部表現に支えられているかを機構的に分析する介入手段を与える。
2. 同一の重みが AR モードと DLM モードを行き来できるかを検証することで、AR モードと DLM モードの本質的差異をアーキテクチャと表現のどちらに帰属させるかを切り分ける。
...
研究は段階的に組み立てた。各 Phase は前 Phase の結果を踏まえて目的を更新する。

### Phase 1: AR ベースラインと五軸評価フレームワーク

最初に AR モデル (Pythia-160M, 410M, 1B) を双方向 attention 化と MDM 損失で DLM 化し、その後 unlearning により AR 挙動を復元できるか検証した。同時に AR モードと DLM モードを公平に評価する五軸評価フレームワークを確立した。
...
| Reverse-context probe (bidirectional) | 先頭 5 トークンを mask、bidirectional モードでの top-1 復元精度 |
| Reverse gap | causal probe と bidirectional probe の差。DLM 固有の双方向参照能力の指標 |
...
Pythia-160M を一旦 DLM 化した後、勾配計算のみで AR ベースラインと統計的に区別できない水準まで復元できた。Pythia-160M と同型の現象は Pythia-410M, Pythia-1B, LLaMA-3.1-8B でも再現を確認している。
...
Phase 1 は自分で DLM 化した AR モデルを AR に戻す実験であり、対象モデルが本来 AR 由来であった点で予備的である。Phase 2 では、はじめから DLM として事前学習された LLaDA-8B-Base (Nie et al. 2025 [4]) を対象とすることでより厳密な検証に進んだ。

評価データセット reversal_v1:

500 個の架空エンティティ対を生成し、forward 方向と reverse 方向の cand@1 を測定する。架空エンティティを用いる理由は、事前学習データに存在しない関係を新規に学習させ、リークの影響を排除するためである。
...
LLaDA は順方向と逆方向で対称な性能を示し、reversal curse (Berglund et al. 2023 [1]) を持たない。AR である Pythia-160M は典型的な逆向き劣化を示す。LLaDA と Pythia-160M の gap 差は +73.5 pp であり、本研究の unlearning が削減を試みる対象である。
...
Unlearning に進む前に、LLaDA に推論時の attention bias として causal mask を強制したら何が起きるかを確認した。上記はアーキテクチャ的には LLaDA の重みのまま AR 風 attention で動かす操作であり、unlearning の上限線として参考になる。
...
表5: Phase 2.5 推論時 causal bias 注入の結果

結果は mode collapse であり、同一重みのまま attention だけを causal にする操作では AR 化は達成されない。LLaDA は推論時の双方向 attention を前提に重みを学習しており、attention 構造を切り替えるだけでは出力分布全体が崩壊する。上記の結果は unlearning が attention 構造を維持したまま重みの方を AR 寄りに調整する必要があることを示唆した。

### Phase 3 v2: 残差ストリーム右側文脈不変性損失 (L_rs)

機構解釈文献に基づき、DLM の双方向参照能力は各層の残差ストリームに右側文脈情報が統合されて流れる挙動として現れるという仮説を立てた。上記の仮説に基づき、自己蒸留型の損失 L_rs を導入した。

L_rs の定義: 入力位置 i で、左側のみ可視の入力と全文脈可視の入力で、各層の hidden state が一致するように forward 側を引き寄せる平均二乗誤差。

```
L_rs = Σ_layer || h_layer^full(i) − h_layer^left_masked(i) ||² / N
```

上記の損失関数を αL_rs + retain·CE の合算として AdamW8bit で 1000 step 最小化する。retain 損失は WikiText-103 上の因果的 next-token CE であり、一般言語能力の保持を担う。
...
Pythia-160M AR の参照点 (+73.5 pp) には届かないが、勾配計算のみによって reverse 側を選択的に劣化させ、forward−reverse 非対称性を生じさせることに成功した。

### Phase 3 v3: 層スライス + マルチポジション化

Transformer 系列の階層別機能分業に関する先行研究 (Tenney et al. 2019 [5], Clark et al. 2019 [2]) は、表層的な構文処理が浅層に、意味的統合が深層に局在することを示している。本研究の予備観察でも、LLaDA の双方向情報統合は層 20 から層 32 までの深層に集中する傾向が認められた。Phase 3 v2 は 33 層を一様平均しており、深層への勾配が約四割に希釈されていた可能性がある。Phase 3 v3 では二点を改良した。
...
表7: Phase 3 v3 の結果 (層スライス + マルチポジション化)

![Phase 2.5 mode collapse と Phase 3 進行](figs/phase3_progress.png)

図4: Phase 2.5 mode collapse と Phase 3 進行

Forward−reverse gap は v2 より拡大したが、forward 側の劣化も同時に進行している。v2 と v3 の訓練曲線比較から、v3 は深層に勾配を集中させることで forget 値が断続的にスパイクしやすく、retain CE もやや高水準で推移することが確認できる。
...
Phase 3 で得られた最も重要な知見は、DLM の右側文脈参照能力が attention 経路と重み記憶経路という少なくとも二つの独立した経路で実現されており、勾配計算による L_rs 最小化は attention 経路を選択的に削減できることである。上記の知見を次の二つの追加実験で示した。

### 並列生成能力の定量化 (parallel_gen_probe)

WikiText-103 validation 上で、各位置 i の元 token を三つの context ablation 条件下で予測した。

- full: 位置 i のみ mask
- left_only: 位置 [i, seq) を mask。AR 等価の左側文脈のみ
- right_only: 位置 [0, i] を mask。右側文脈のみ

右側文脈の寄与を log p_full − log p_left_only で定義する。上記の指標は位置 i で元 token を当てるとき、右側文脈が左側文脈にどれだけ追加情報を与えているかを nats で測る。
...
右側文脈の寄与が約 94% 削減された。上記の削減は並列にトークンを生成する能力の主要な物理基盤である右側の masked 位置からの情報還流が実質的に消失したことを示す。
...
Full − no_Y の差分は Y への attention 経路の寄与を、no_Y − no_context の差分は重み記憶経路の寄与をそれぞれ定量化する。
...
表9: 記憶経路と attention 経路の寄与分解

First-token cand@1 では Y を可視にしたときの上昇分が attention 経路、Y を mask しても残る予測精度が重みベースの寄与に対応する。L_rs は Y attention 経路を 70% 削減した一方、重み記憶の水準はほぼ動いていない。重要なのは、span 単位の reverse cand@1 が 0.705 と高水準にとどまる点であり、上記の事実は主体名 X 全体の重み記憶が L_rs では削減されないことを意味する。L_rs は attention 経路 (Y → X) を主に削減し、X の固有名詞自体の重み記憶は無傷で残すという二重経路像が確認された。

![記憶経路と attention 経路の分解](figs/memorization_paths.png)

図7: 記憶経路と attention 経路の分解

## 限界

本研究は L_rs による右側文脈 attention 経路の選択的削減を示したが、次の限界がある。

1. 完全な AR 復元には至らない: Phase 3 v3 の +14 pp は AR 参照点 +73.5 pp の約二割であり、forward−reverse 非対称性は部分的にしか再現されない。
2. Forward 側の collateral damage: forward cand@1 も 0.995 から 0.845 に劣化した。log p_full は Phase 2 の -0.86 から v3 では -5.91 まで下がり、left_only でさえ Phase 2 baseline の -4.06 を下回る。上記の劣化は左側文脈のみで予測する能力自体が同時に弱まっていることを示す。
3. 重み記憶経路の unlearning は未達: L_rs は attention 経路を消すが、weights に焼き込まれた reverse 関係は残る。上記の事実は reversal curse の真の機構解明には別の介入が必要であることを示唆する。
4. 実応用との接続が不明瞭: 右側文脈参照を消す操作の実用的価値は明確でない。本研究で得た勾配計算による経路選択的削減の知見は後続研究の出発点として保持される。
...
src/unlearning_architecture/
    ...
    eval.py             # 五軸評価の中核
    data.py             # WikiText-103 / 合成関係データ streaming loader
...
scripts/
    ...
    parallel_gen_probe.py              # 並列生成能力の定量化
...
data/
    reversal_v1/        # 500 の架空エンティティ対からなる forward/reverse 評価データ
...
代表的なコマンド例を以下に示す。GPU は CUDA_VISIBLE_DEVICES で個別指定し、長時間の run は nohup で起動する。
...
- [figs/wandb_p1_unlearn_b4.png](figs/wandb_p1_unlearn_b4.png): Phase 1 unlearning B4 構成の loss/forget/retain
- [figs/wandb_p2_llada_sft.png](figs/wandb_p2_llada_sft.png): Phase 2 LLaDA-8B-Base の reversal_v1 SFT (2k step)
- [figs/wandb_p3v2_lrs.png](figs/wandb_p3v2_lrs.png): Phase 3 v2 の L_rs unlearning (uniform 33 層)
