...
拡散言語モデル(DLM)を重みの勾配更新のみでAR(自己回帰)モデルの挙動に戻せるか検討した研究リポジトリ。

## TL;DR

- AR由来のDLM(Pythia系)はアンラーニングだけでARベースラインまで完全復元できた
- ネイティブDLM(LLaDA-8B-Base)では順逆差を+1.5 ppから+14 ppまで拡大したが、AR参照点+73.5 ppには未到達
- DLMの右側文脈参照はアテンション経路と重み記憶経路の二重経路で実現されており、L_rs損失はアテンション経路を選択的に約70%削減する一方、重み記憶経路は無傷で残る
